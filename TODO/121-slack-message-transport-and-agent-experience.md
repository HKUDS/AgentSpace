...
- strict final evidence 报告现在有顶层 `blockers` 聚合字段，会直接暴露 `active_slack_integration_missing`、`slack_live_smoke_evidence_missing`、integration 级 blockers 和 live artifact / 本地 evidence integration mismatch，方便验收自动化不用解析嵌套字段也能判断失败原因。
