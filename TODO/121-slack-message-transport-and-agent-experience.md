...
- strict Slack evidence 现在也会把 pending outbox 作为 `pending_outbox_unresolved` 阻断，确保 live app mention 后必须先执行 outbox drain 并完成 Slack thread 回写，再运行最终 gate。
...
- `smoke-plan` 在尚未创建 Slack integration 时会把 `create` 命令绑定到当前 workspace，并在 `health-check` / `bind-channel` / `bind-user` / `outbox drain` 里输出 `CHANGE_ME_SLACK_INTEGRATION_ID` 占位，避免按计划执行验收时复制到缺少 `--integration` 的无效命令。
