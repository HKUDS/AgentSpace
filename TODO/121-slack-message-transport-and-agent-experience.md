...
> 更新时间：2026-07-04
> 状态：实现中，分支 `codex/slack-integration` 已落地 MVP provider / Events API / CLI 管理入口 / Settings 管理入口
...
- 单元测试覆盖签名、challenge、归一化、outbound、route、CLI help、settings data/action、settings client 兼容性、socket worker 和 health/readiness/smoke dry-run。
...
- live Slack smoke 直接发送 disposable channel message。
