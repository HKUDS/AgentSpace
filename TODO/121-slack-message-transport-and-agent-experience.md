...
- Slack files 完整 live smoke evidence；可选增强为接入真正病毒扫描引擎。
...
- live smoke evidence artifact 会记录 AgentSpace workspace / integration context，以及 redacted app/team reference；strict final evidence 只接受与当前 workspace/integration 匹配的 live proof，且 workspace-wide 检查时 live proof 和本地 evidence 必须指向同一个 integration，避免复用其他 Slack 集成的 artifact 误通过。
