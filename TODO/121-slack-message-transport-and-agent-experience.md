...
- Health/readiness/smoke/evidence 辅助：`auth.test`、`apps.connections.open` dry check、scope/header 校验和 manual review、`readiness|smoke-plan|smoke-env|evidence` CLI、`scripts/slack/smoke.ts --check-env` dry-run；`smoke-plan` 输出 Slack `features.agent_view` manifest 草案，`evidence` 从本地 event/mapping/outbox/binding 记录汇总 message/native/approval 验收信号。
...
- [x] CLI 支持 `evidence` 从本地 event/mapping/outbox/binding 记录汇总 message/native/approval 验收信号。
...
- [x] evidence report 不输出原始 Slack app/team/channel/user/message/event id，并能按 `message|native|approval|all` 严格门禁退出。
...
14. 本地 `slack evidence --strict --require all` 可证明 message/native/approval 信号，且不泄露原始 Slack ID。
