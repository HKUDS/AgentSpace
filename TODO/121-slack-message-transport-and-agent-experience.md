...
- final evidence CLI 现在会区分 live smoke artifact 缺失、读取失败和 JSON 损坏；坏 artifact 会报告 `slack_live_smoke_evidence_read_failed` / `slack_live_smoke_evidence_json_invalid`，避免误判成缺少 live proof。
...
- `scripts/slack/smoke.ts --check-env` 也会把 `SLACK_SMOKE_APP_ID` / `SLACK_SMOKE_TEAM_ID` 当作必填的非 secret 验收上下文，避免 dry-run 先通过、live artifact 阶段才发现缺少 final evidence 所需 app/team hash。
...
- `smoke:slack:verify` JSON 现在会输出 `manualActions` 和 `nextCommands`：前者提醒 final `--require all` 前还要完成 native agent experience 与 approval block action proof，后者在 artifact 通过时提示 outbox drain + final evidence；缺 live mode 时只提示补缺 mode；频道/context/泄露等 artifact 质量问题会提示按顺序重跑三段 live smoke、drain、re-verify 和 final evidence，避免验收流程卡在 verifier 输出后。
- `smoke:slack:verify` 针对自定义 `--verify-evidence` / `--env-file` path 生成 remediation 时会保留同一个 artifact 与 env path，避免 `npm run smoke:slack:verify` 的默认 `runtime-output/slack-smoke/live.json` 或 `scripts/slack/.env` 覆盖临时验收上下文。
- final evidence remediation 针对自定义 `--live-smoke-evidence` path 也会保留同一个 artifact：顶层和 `integrations[].nextCommands` 的三段 live smoke、`smoke:slack:verify --verify-evidence ...`、最终 `evidence --live-smoke-evidence ...` 都不会退回默认 `runtime-output/slack-smoke/live.json`。
- 自定义 env / live evidence path 出现在 `smoke:slack:verify` 和 final evidence remediation commands 时现在会按需 shell-quote，避免临时目录或 artifact 文件名带空格时复制命令被 shell 拆坏。
- smoke harness 和 AgentSpace CLI parser 现在都支持 `--flag=value`，路径以 `-` 开头时 remediation commands 会生成等号形式，避免 `--verify-evidence --custom.json` / `--live-smoke-evidence --custom.json` 被误解析成布尔 flag。
- smoke dry-run 与 `smoke:slack:verify` 的 remediation commands 现在会优先使用 env-file 里解析出的 AgentSpace workspace / Slack integration id；只有 env 缺失时才保留 `$AGENT_SPACE_WORKSPACE_ID` / `$AGENT_SPACE_SLACK_INTEGRATION_ID` 占位，避免操作者没有 export `.env` 就直接复制 outbox drain / final evidence 命令导致验收跑偏。
- `smoke:slack:verify` 与 strict final evidence 现在都会识别旧版单次 live-run artifact；旧 artifact 仍无法满足 `--require all`，但会按已存在的 live mode 给出准确 satisfied/missing 诊断，而不是误报 `runs_missing`。
- `smoke-plan`、`smoke-env` 和 final evidence remediation 的 next commands 都包含 `npm run smoke:slack:verify -- --env-file scripts/slack/.env --json`，避免操作者跳过 live artifact 离线校验；final evidence 顶层与 `integrations[].nextCommands` 现在都会先提示 `smoke-env`、`health-check` 和 `--check-env`，确保 strict gate 所需的 fresh health-check 与 live smoke `.env` 前置条件不会被漏掉。
...
- final evidence 和 `smoke:slack:verify` 现在要求 `post_message`、`app_mention`、`file_upload` 三种 live run 的 `channelReference` 指向同一个脱敏 channel；跨频道拼接历史 run 会以 `slack_live_smoke_channel_mismatch` 阻断。
- strict final evidence 现在还会把 live `app_mention` 的脱敏 `messageRef` + `channelReference` 同本地 inbound mapping、sent outbox / outbound mapping 双键关联；只有 thread/message 对上但 channel 不对会以 `slack_live_app_mention_channel_correlation_missing` 阻断。
...
- Slack message evidence 的基础门禁现在还要求 agent reply 的 outbound mapping / sent outbox 与带 task queue proof 的 inbound mapping 落在同一 Slack thread；不同历史 inbound/outbound 片段不能拼接满足 message gate，缺失时会返回 `thread_correlated_outbound_reply_evidence_missing`。
- `smoke-plan` 的结构化 checklist 和 final evidence 的 top-level `nextCommands` 现在都会在 `live_app_mention` 后显式给出 `agent-space integrations slack outbox drain ... --json`，避免最终验收前漏掉 Slack thread reply 回写。
...
- `smoke-env` 生成的 `.env` 模板和 `scripts/slack/env.example` 现在也把 manual actions 与 health/readiness/check-env/replay/live/outbox/file-upload/verify/final evidence 顺序写成注释，重定向保存后仍能看到完整验收链路且不会影响 env parser。
- `scripts/slack/smoke.ts --check-env` 的 JSON 和 text 输出现在也会展示同一组 `manualActions`，让直接从 smoke harness 进入验收的操作者不会漏掉 native / approval 的人工 proof。
...
- strict Slack native evidence 现在还要求 sent app-home welcome 和 sent assistant suggested prompts 带有相同 `externalUserReference`；不同 Slack 用户/历史片段的 native outbox 不能拼接满足 native gate，缺失时会返回 `native_user_experience_correlation_missing`。
...
- strict Slack files evidence 现在还要求 sent `slack_file_upload` outbox 与已存储附件的 inbound mapping 落在同一 Slack channel/thread；只有孤立出站上传证据会以 `slack_file_upload_thread_correlation_missing` 阻断。
- strict Slack files evidence 现在还会把 live `file_upload` 的脱敏 `channelReference` 与本地已 thread 关联的 file upload proof 对齐；live artifact 和本地文件上传证据频道不一致会以 `slack_live_file_upload_channel_correlation_missing` 阻断。
...
- Slack CLI `health-check` 的非 JSON text 输出现在使用专用摘要，展示 status、scope/socket 状态、checks、detail 和 nextStep，同时避免 raw Slack app/team/user id 出现在 text 摘要里。
- Slack CLI `readiness` 的非 JSON text 输出现在使用专用摘要，显式展示每个 integration 的 message/worker readiness、bindings/outbox、blockers/warnings 和 next commands，避免嵌套对象渲染影响前置排障。
...
- Slack CLI `smoke-plan` 的 text 输出现在也列出 `create`、`bindChannel`、`bindUser`、dry smoke 和 webhook replay 命令，避免不使用 `--json` 时漏掉 setup / binding / replay 步骤。
- Slack CLI `evidence` 的 text 输出现在完整列出 remediation `nextCommands`，不会在 `file_upload`、`smoke:slack:verify` 和最终 `--live-smoke-evidence ... --strict --require all` 前截断。
