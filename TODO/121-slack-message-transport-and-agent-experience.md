...
- Slack files 入站下载、AgentSpace attachment storage、大文件/安全扫描策略。
...
后续增强，不阻塞 MVP。当前入站已完成 metadata-only 安全摘要，真实 Slack 文件内容仍不进入 AgentSpace；出站已支持 AgentSpace 正式附件走 Slack external upload flow。
