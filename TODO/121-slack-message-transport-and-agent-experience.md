...
- Slack CLI `create` / `bind-agent-bot` / `disable-agent-bot` 的用户可见摘要现在只输出 `appReference` / `teamReference`，不再回显 raw Slack app id 或 team id；`apps/cli/src/commands/integrations.test.ts` 覆盖这些 JSON 输出脱敏。
