...
- `smoke:slack:verify` JSON 现在会输出 `nextCommands`：artifact 通过时提示 outbox drain + final evidence；缺 live mode 时只提示补缺 mode；频道/context/泄露等 artifact 质量问题会提示按顺序重跑三段 live smoke、drain、re-verify 和 final evidence，避免验收流程卡在 verifier 输出后。
