...
- Health/readiness/smoke 辅助：`auth.test`、`apps.connections.open` dry check、scope/header 校验和 manual review、`readiness|smoke-plan|smoke-env` CLI、`scripts/slack/smoke.ts --check-env` dry-run；`smoke-plan` 输出 Slack `features.agent_view` manifest 草案。
...
- Slack app home onboarding：`app_home_opened` 的 Messages tab 首次打开会幂等排队一条 welcome Block Kit 消息，通过 outbox 发送；mapping / outbox metadata 只保存 redacted reference。
...
- agent-scoped Slack bot 的多 bot 同频道/线程协作验收、`agent_view` runtime suggested prompts。
