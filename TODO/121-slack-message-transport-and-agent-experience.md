...
- strict Slack approval evidence 现在也要求 `block_actions` 处理记录和 `agent_status_card` outbox 状态回写同时存在；只有回调处理成功但没有把审批结果回写 Slack thread 时，`--require approval/all` 会以 `slack_approval_status_outbox_evidence_missing` 阻断。
