...
- `smoke-plan` 在尚未创建 Slack integration 时会把 `create` 命令绑定到当前 workspace，并在 `smoke-env` / `health-check` / `readiness` / `bind-channel` / `bind-user` / `outbox drain` 里输出 `CHANGE_ME_SLACK_INTEGRATION_ID` 占位，避免按计划执行验收时复制到缺少 `--integration` 的无效命令。
