...
- `smoke:slack:verify` 针对自定义 `--verify-evidence` path 生成 remediation 时会保留同一个 artifact path，避免 `npm run smoke:slack:verify` 的默认 `runtime-output/slack-smoke/live.json` 覆盖临时验收文件路径。
