...
> - `smoke-plan` / evidence remediation 已补 bound user direct mention 审计要求：`live_agent_bot_direct_mention` 会要求从已绑定飞书用户直接 @具体 agent bot，并验证 `actorType=user`、`actorUserId`、安全审计引用、agentId、botBindingId、task 和 message evidence，避免只证明路由成功却漏掉“真实 user actor 和审计”的 Phase 6 验收点。
...
> - Final evidence gate 已补 failed outbox failure visibility 校验：失败 outbox 必须属于当前 agent bot binding，带 safe `externalChatReference`，且不能含 raw external/target chat/thread id，避免无关 bot 或泄漏目标 id 的失败行被算作“失败可见且可归因”。
...
- [x] Failure visibility evidence 已收紧：provider failure row 必须与 degraded/error health 同时出现，且 agent bot failure 证据必须带 Feishu provider、agentId、botBindingId 和安全 chat/resource context；failed outbox 需要 `outboxSource` + `externalChatReference`，failed data operation 需要 Feishu governance context + resource binding id。最终证据 gate 使用 `agent_bot_failure_with_safe_context`，避免用无上下文失败误证明“失败对管理员可见且可归因到具体 agent bot”。
