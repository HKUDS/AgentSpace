...
> - AgentSpace `evidence` 已补 report-level setup remediation：当 workspace 还没有可用 Feishu agent bot integration，或 `--integration` 指向不存在的 binding 时，JSON / 可读输出会显式暴露 `integration_missing` / `selected_integration_missing`，回显 selected integration，并优先给出 `smoke-plan` 与 `bind-agent-bot` 下一步命令；每个 integration evidence item 也会显示 `status`，避免管理员直接跑 final evidence 时只看到 OpenAPI / bot-added artifact 缺失，或把 disabled / archived 旧 binding 当成可用证据来源。
...
> - CLI `create` / `bind-agent-bot` / `agent-channel-access` 的 `nextCommands.smokePlan` 与 `finalEvidence` 已同步可读排障默认值，`smoke-plan` 里展示的最终 AgentSpace evidence 命令也不再默认追加 `--json`；health/readiness/OpenAPI verifier 这类机器消费命令仍保留 JSON 输出。
